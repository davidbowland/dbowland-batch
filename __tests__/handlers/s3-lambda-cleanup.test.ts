...
import { lambdaCleanupProject, s3Object, s3ObjectEvenOlder, s3ObjectOlder } from '../__mocks__'
...
describe('s3-lambda-cleanup', () => {
  beforeAll(() => {
    ...
    mocked(processPromiseQueue).mockImplementation(async (promiseFn, iterable) => { await Promise.all(iterable.map(promiseFn)) })
  ...
  describe('s3LambdaCleanup', () => {
    test('expect the second-oldest template file and older files are deleted', async () => {
      ...
      expect(s3.listS3Objects).toHaveBeenCalledWith(s3.s3ClientEast1, lambdaCleanupProject.bucket, lambdaCleanupProject.prefixes[0])
      ...
      expect(s3.deleteS3Object).toHaveBeenCalledWith(s3.s3ClientEast1, lambdaCleanupProject.bucket, s3ObjectEvenOlder.key)
      expect(s3.deleteS3Object).toHaveBeenCalledTimes(2)
      expect(processPromiseQueue).toHaveBeenCalledWith(expect.anything(), expect.anything(), { concurrency: s3LambdaCleanupNumberOfThreads })
    })

    test('expect us-east-2 client in us-east-2 region', async () => {
      const usEast2Project: LambdaCleanupProject = { ...lambdaCleanupProject, prefixes: ['lambda-one/', 'lambda-two/'],  region: 'us-east-2' }
      ...
      expect(s3.deleteS3Object).toHaveBeenCalledWith(s3.s3ClientEast2, lambdaCleanupProject.bucket, s3ObjectEvenOlder.key)
      expect(s3.deleteS3Object).toHaveBeenCalledTimes(2)
    })

    test('expect invalid region rejects', async () => {
      const invalidRegionProject: LambdaCleanupProject = { ...lambdaCleanupProject, region: 'invalid-region' as LambdaRegion }
    ...
    test('expect no files deleted when second-oldest template files exist', async () => {
